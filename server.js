...
items.forEach((it) => {
  ...
  lines.push(${it.qty} x ${it.name}  € ${tot}${it.notes ? "\n  NOTE: " + it.notes : ""});
    ...
    // scrivi file su disco (opzionale)
...
app.get("/:restaurant/table/:code", (req, res) => {
  ...
  const t = db.data.tables.find(
    (x) => x.restaurant_id === r.id && x.code === code
  );
...
app.post("/:restaurant/table/:code/order", async (req, res) => {
  ...
  for (const it of items) {
    const p = db.data.products.find(
      (x) => x.id === it.product_id && x.restaurant_id === r.id
    );
    if (!p) continue;
  ...
  if ((pay_method || "cassa") === "online") {
    ...
    if (sumupPrefix) {
      ...
      return res.redirect(
        ${sumupPrefix}?amount=${eur}&label=Order%20${o.code}
      );
    } else {
...
app.get("/admin", requireAdmin, (req, res) => {
  const restaurants = [...db.data.restaurants].sort((a, b) =>
