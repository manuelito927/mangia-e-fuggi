...
function requireAdmin(req,res,next){ if(req.session.admin) return next(); res.redirect('/login') }
...
async function printOrder(orderId){
  try{
    ...
    lines.push('-----------------------------')
    items.forEach(it=> lines.push(`${it.qty} x ${it.name}  € ${(it.price_cents*it.qty/100).toFixed(2)}${it.notes? '\n  NOTE: '+it.notes : ''}`))
    lines.push('-----------------------------')
    lines.push(`TOTALE: € ${euro(o.total_cents)}`)
    lines.push(`PAGAMENTO: ${o.pay_method}`)
    const payload = lines.join('\n') + '\n\n'

    // Write to spool dir if enabled
    ...
    // Optional: send to PrintNode if configured
    if(process.env.PRINTNODE_API_URL && process.env.PRINTNODE_API_KEY){
      try{
        const url = process.env.PRINTNODE_API_URL
        ...
        await res.text()
      }catch(e){ console.error('PrintNode error', e.message) }
    }
  }catch(e){
...
// ---- MENU (foto sinistra, prezzo/qty destra) ----
app.get('/:restaurant/table/:code', async (req,res)=>{
  ...
  // foto di categoria (royalty-free da Unsplash Source)
  const catImg = (name)=>{
...
app.post('/:restaurant/table/:code/order', async (req,res)=>{
  ...
  db.data.orders.push({ id, restaurant_id:r.id, table_id:t.id, code:oc, status:'Nuovo', pay_method: (pay_method||'cassa'), subtotal_cents:0, total_cents:0, created_at:nowISO(), paid_at:null })
  let subtotal = 0
  for(const it of items){
    ...
    db.data.order_items.push({ id: uuidv4(), order_id:id, product_id:p.id, name:p.name, qty, price_cents:p.price_cents, notes:(it.notes||'') })
  }
  const o = db.data.orders.find(x=>x.id===id); o.subtotal_cents=subtotal; o.total_cents=subtotal; await db.write()

  // stampa comanda
  await printOrder(id)

  // pagamento online (mock o SumUp link se configurato)
  if((pay_method||'cassa')==='online'){
    const sumupPrefix = process.env.SUMUP_PAYMENT_LINK_PREFIX || ''
    if(sumupPrefix){
      // redireziona a SumUp hosted link (se impostato)
...
// ---- MOCK PAY (se non configurato SumUp) ----
...
app.post('/order/:id/pay/mock-success', async (req,res)=>{
  ...
  o.status='Pagato'; o.paid_at=nowISO(); await db.write()
...
app.post('/login', (req,res)=>{
  ...
  if(a){ req.session.admin={ id:a.id, username:a.username }; res.redirect('/admin') }
  else res.render('login', { error:'Credenziali non valide' })
})
...
app.post('/admin/:slug/products', requireAdmin, async (req,res)=>{
  ...
  db.data.products.push({ id, restaurant_id:r.id, category_id:req.body.category_id, name:req.body.name, description:(req.body.description||''), price_cents:cents, img:null })
  await db.write(); res.redirect(`/admin/${r.slug}`)
})

// Upload foto prodotto
app.post('/admin/product/:id/photo', requireAdmin, upload.single('photo'), async (req,res)=>{
  const { id } = req.params
  const p = db.data.products.find(x=>x.id===id); if(!p) return res.status(404).send('not found')
