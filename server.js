// server.js — MANGIA&FUGGI (full clean copy)
...
app.get('/:restaurant/table/:code', (req, res) => {
  ...
  // immagini di categoria (royalty-free via Unsplash Source)
  const catImg = (name) => {
    ...
    if (n.includes('dolci')) return 'https://source.unsplash.com/600x400/?dessert,italy';
...
app.post('/:restaurant/table/:code/order', async (req, res) => {
  ...
  // pagamento online (mock o link SumUp se configurato)
...
app.get('/admin/:slug', requireAdmin, async (req, res) => {
  ...
  const qrs = await Promise.all(
    tables.map(async t => {
      const url = ${BASE_URL}/${r.slug}/table/${t.code};
...
app.post('/:restaurant/table/:code/call', (req, res) => {
  ...
  if (process.env.PRINT_TO_FILES === 'true') {
    const dir = process.env.PRINT_SPOOL_DIR || './spool';
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const name = path.join(dir, call_${t.code}_${Date.now()}.txt);
